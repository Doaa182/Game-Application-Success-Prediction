...
df = pd.DataFrame() 
#URL
# df=pd.concat([df,data['URL']], axis=1)
# data = data.drop(columns=["URL"])

#subtitle
# df=pd.concat([df,data['Subtitle']], axis=1)
# data = data.drop(columns=["Subtitle"])
...
df=pd.concat([df,data['In-app Purchases']], axis=1)
data = data.drop(columns=["In-app Purchases"])
...
df=pd.concat([df,data['Age Rating']], axis=1)
data = data.drop(columns=["Age Rating"])
...
df=pd.concat([df,data['Original Release Date']], axis=1)
...
df=pd.concat([df,data['Current Version Release Date']], axis=1)
...
# df=pd.concat([df,data['Average User Rating']], axis=1)
# data = data.drop(columns=["Average User Rating"])
...
pickle.dump(linear_lasso_reg, open(file_name_lassoLinear, 'wb'))
...
pickle.dump(linear_ridge_reg, open(file_name_ridgeLinear, 'wb'))
# Predict on the test data using the polynomial features
...
###############################################################################
###############################################################################
file_name_wrapper='wrapper_model.sav'
#Forward (wrapper)
...
model_wrapper=linear_forward_reg.fit(Forward_X,Y)
pickle.dump(model_wrapper, open(file_name_wrapper, 'wb'))
# Predict on the test data using the polynomial features
loaded_model_wrapper = pickle.load(open(file_name_wrapper, 'rb'))
prediction = loaded_model_wrapper.predict(Forward_X)
...
model=linear_reg.fit(X_train_poly, Y_train)
pickle.dump(model, open(file_name_poly_lasso, 'wb'))
# Predict on the test data using the polynomial features
...
model=ridge_poly.fit(X_train_poly, Y_train)
pickle.dump(model, open(file_name_poly, 'wb'))
# Predict on the test data using the polynomial features
